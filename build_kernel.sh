...
if [ "$ENABLE_KSU" -eq 1 ]; then
    echo "==========================================="
    echo " [*] Applying SukiSU Manager v3/v4 Compat Patch"
    echo "==========================================="
...
def patch_allowlist(p: Path):
    if not p.exists():
        print(f"[-] allowlist: file not found: {p}")
        return
    t = p.read_text()
...
def patch_ksud(p: Path):
    if not p.exists():
        print(f"[-] ksud: runtime/ksud.c missing at {p}; skip throne fallback")
        return
    t = p.read_text()
...
def patch_dispatch(p: Path):
    if not p.exists():
        print(f"[-] dispatch: file not found: {p}")
        return
    t = p.read_text()
